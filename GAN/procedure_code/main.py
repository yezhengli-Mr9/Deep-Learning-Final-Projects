...
def main(config):
  #img_name = './000798.jpg'
  #img_name = '/Users/apple/Desktop/Penn2/CIS680/HW3/sample_data/test_data/P&C_dataset_imgs/img/000000.jpg'
  #read_img = matplotlib.image.imread(img_name)
  #print(read_img)
  ...
  if config.is_train:
    test_data_loader, test_label_loader_list = get_loader(
      config.data_path, config.batch_size_test, config.testset_start_index, 'test', False)
  else:
    test_data_loader, test_label_loader_list = get_loader(
      config.data_path, config.batch_size_test, 0, config.split, False)
