...
def conv_factory(x, hidden_num, kernel_size, stride, is_train, reuse, with_rec=True, activation='relu'):
...
def transpose_conv_factory(x, hidden_num, kernel_size, stride, is_train, reuse, with_rec=True):
  vs = tf.get_variable_scope()
  x  = tf.layers.conv2d_transpose(x, filters=hidden_num, kernel_size=kernel_size,
                          strides=[stride, stride], padding='SAME', activation=None)
  if with_rec:              
      x = batch_norm(x, is_train=is_train)
      x = tf.nn.relu(x)
  return x  
  
def fc_factory(x, hidden_num, is_train, reuse, with_rec=True):
